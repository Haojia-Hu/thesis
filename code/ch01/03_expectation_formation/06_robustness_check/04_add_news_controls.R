# ==========================================================
# Local Projection: Robustness Check 04
# Robustness: Add news controls (Factiva)
#
# Purpose:
#   Local Projections with news control (Diff_News) added.
#
# Input:
#   - data/transformed/ch01/panel/panel_data.csv
#
# Differences vs baseline:
#   - Add Diff_News to RHS controls
#
# Outputs:
...
out_dir <- "data/output/ch01/local_projection/robustness_check/with_news_controls"
dir.create(out_dir, recursive = TRUE, showWarnings = FALSE)

out_irf  <- file.path(out_dir, "irf_with_news_controls.csv")
out_coef <- file.path(out_dir, "coef_path_with_news_controls.csv")
fig_both <- file.path(out_dir, "fig_irf_interactions_with_news_controls.png")
...
if (length(missing_cols) > 0) {
  stop(
    "Missing required columns in panel data: ",
    paste(missing_cols, collapse = ", "),
    "\nIf Diff_News is missing, ensure it is constructed in the panel build script."
  )
}
...
lp_results_news <- map(horizons, function(h) {
...
irf_pc  <- get_irf_df(lp_results_news, "Interaction_PC",  "Attention × Price Change")
irf_vol <- get_irf_df(lp_results_news, "Interaction_Vol", "Attention × Price Volatility")
irf_att <- get_irf_df(lp_results_news, "L1_diff_index",   "Attention (L1)")

irf_all <- bind_rows(irf_att, irf_pc, irf_vol)
write_csv(irf_all, out_irf)

coef_path <- map2_dfr(lp_results_news, horizons, function(model, h) {
