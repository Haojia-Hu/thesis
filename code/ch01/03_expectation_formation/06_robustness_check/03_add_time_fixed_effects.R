# ==========================================================
# Local Projection: Robustness Check 03
# Robustness: Add time fixed effects
#
# Purpose:
#   Local Projections with Time Fixed Effects (category + Date FE).
#
# Input:
#   - data/transformed/ch01/panel/panel_data.csv
#
# Differences vs baseline:
#   - Add Date fixed effects: | category + Date
#   - Other regressors unchanged (lagged attention + interactions + macro controls)
#
# Outputs:
...
out_dir <- "data/output/ch01/local_projection/robustness_check/time_fixed_effects"
dir.create(out_dir, recursive = TRUE, showWarnings = FALSE)

out_irf  <- file.path(out_dir, "irf_time_fixed_effects.csv")
out_coef <- file.path(out_dir, "coef_path_time_fixed_effects.csv")
fig_both <- file.path(out_dir, "fig_irf_interactions_time_fixed_effects.png")
...
# ==========================
# 2) Construct lag + interactions (within category)
# ==========================
panel_df1 <- panel_df %>%
  group_by(category) %>%
  arrange(Date) %>%
...
lp_results_fe <- map(horizons, function(h) {
...
irf_pc  <- get_irf_df(lp_results_fe, "Interaction_PC",  "Attention × Price Change")
irf_vol <- get_irf_df(lp_results_fe, "Interaction_Vol", "Attention × Volatility")
irf_att <- get_irf_df(lp_results_fe, "L1_diff_index",   "Attention (L1)")

irf_all <- bind_rows(irf_att, irf_pc, irf_vol)
write_csv(irf_all, out_irf)

coef_path <- map2_dfr(lp_results_fe, horizons, function(model, h) {
...
p <- ggplot(irf_interactions, aes(x = h, y = estimate, shape = variable)) +
  geom_line(aes(group = variable), color = "black", linewidth = 1.0) +
  geom_point(size = 2) +
